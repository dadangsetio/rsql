...
export function OpenDatabaseTabs({
  onEditConnection,
}: {
  ...
  const [pgBackupTarget, setPgBackupTarget] = React.useState<{
    mode: "backup" | "restore";
    projectId: string;
    dbName: string;
  } | null>(null);
    ...
    <div className="flex items-center gap-0.5 overflow-x-auto overflow-y-hidden whitespace-nowrap scrollbar-none border-b border-sidebar-border px-1.5 py-1">
                  ...
                  ? [
                      {
                        ...
                        onClick: () => void useProjectStore.getState().refreshConnection(pid),
                      },
                      {
                        label: "Performance Monitor",
                        icon: <Activity className="h-3 w-3" />,
                        onClick: () => openMonitorTab(pid),
                      },
                      {
                        label: "PG Settings",
                        icon: <Settings className="h-3 w-3" />,
                        onClick: () => openPgSettingsTab(pid),
                      },
                      {
                        label: "LISTEN/NOTIFY",
                        icon: <Zap className="h-3 w-3" />,
                        onClick: () => openNotifyTab(pid),
                      },
                      {
                        label: "Schema Diff",
                        icon: <Columns3 className="h-3 w-3" />,
                        onClick: () => openSchemaDiffTab(pid),
                      },
                      {
                        label: "Extensions",
                        icon: <Package className="h-3 w-3" />,
                        onClick: () => openExtensionsTab(pid),
                      },
                      {
                        label: "Enum Types",
                        icon: <List className="h-3 w-3" />,
                        onClick: () => openEnumsTab(pid),
                      },
                      { separator: true as const },
                      {
                        ...
                        onClick: () =>
                          setPgBackupTarget({ mode: "backup", projectId: pid, dbName: label }),
                      },
                      {
                        label: "Restore...",
                        icon: <Upload className="h-3 w-3" />,
                        onClick: () =>
                          setPgBackupTarget({ mode: "restore", projectId: pid, dbName: label }),
                      },
                    ]
                  : []),
                ...(onEditConnection
      ...
      {pgBackupTarget && (
        <PgBackupModal
          open={!!pgBackupTarget}
          onOpenChange={(open) => {
            if (!open) setPgBackupTarget(null);
          }}
